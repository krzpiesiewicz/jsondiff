...
class JsonDiffTests(unittest.TestCase):
    ...
    def test_a(self):
        ...
        self.assertEqual('1 -> 2', diff(1, 2))
        self.assertEqual('True -> False', diff(True, False))
        self.assertEqual('abc -> def', diff('abc', 'def'))
        ...
        self.assertEqual("{'a': 1, 'b': 2} -> 123", diff({'a': 1, 'b': 2}, 123))

        self.assertEqual({delete: ['b']}, diff({'a': 1, 'b': 2}, {'a': 1}))
        self.assertEqual({'b': '2 -> 3'}, diff({'a': 1, 'b': 2}, {'a': 1, 'b': 3}))
        ...
        self.assertEqual(
            {insert: [(2, 'b')], delete: [4, 0], 1: {'v': '11 -> 20'}},
    ...
    def test_compact_syntax(self, scenario):
        ...
        if type(a) in [int, str]:
            self.assertEqual(f'{a} -> {b}', f"{differ.patch(a, d)}")
        else:
